...
export function sanitizeFeedbackText(value: string | null | undefined): string {
  return capitalizeLeadingLetter(collapseWhitespace(
    String(value ?? "")
      ...
      .replace(/\baccording to (?:the )?data provided(?:\s+on\s+[^,.;:!?]+)?[,;:]?\s*/gi, "")
      .replace(/\baccording to (?:the )?provided (?:evidence|material|source),?\s*/gi, "")
      .replace(/\baccording to (?:the )?provided data,?\s*/gi, "")
      .replace(/\bbased on (?:the )?(?:evidence|material|source) provided,?\s*/gi, "")
      .replace(/\bbased on (?:the )?data provided(?:\s+on\s+[^,.;:!?]+)?[,;:]?\s*/gi, "")
      .replace(/\bbased on (?:the )?provided (?:evidence|material|source),?\s*/gi, "")
      .replace(/\bbased on (?:the )?provided data,?\s*/gi, "")
      .replace(/\bbased on (?:the )?source,?\s*/gi, "")
      .replace(/\bthe (?:data|evidence) provided (?:show|shows|mention|mentions|state|states|indicate|indicates|describe|describes|explain|explains|note|notes) that\s*/gi, "")
      .replace(/\bthe (?:data|evidence) provided (?:show|shows|mention|mentions|state|states|indicate|indicates|describe|describes|explain|explains|note|notes)\s*/gi, "")
      ...
      .replace(/\bin the cited material,?\s*/gi, "")
      .replace(/\bfrom the cited material,?\s*/gi, "")
      .replace(/\bthe cited evidence (?:mentions?|states?|indicates?|shows?|describes?|explains?|notes?) that\s*/gi, "")
      .replace(/\bthe cited evidence (?:mentions?|states?|indicates?|shows?|describes?|explains?|notes?)\s*/gi, "")
      .replace(/\bthe cited evidence,?\s*/gi, "")
      .replace(/\bfrom the source,?\s*/gi, "")
      .replace(/\bin the source material,?\s*/gi, "")
      .replace(/\baccording to the excerpts?,?\s*/gi, "")
      .replace(/\bbased on the excerpts?,?\s*/gi, "")
      .replace(/\bin the excerpts?,?\s*/gi, "")
      .replace(/\bfrom the excerpts?,?\s*/gi, "")
      ...
      .replace(/\baccording to the source material,?\s*/gi, "")
      .replace(/\bbased on the source material,?\s*/gi, "")
      .replace(/\bfrom the source material,?\s*/gi, "")
