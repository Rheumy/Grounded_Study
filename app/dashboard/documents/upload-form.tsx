...
const INTERRUPTED_UPLOAD_MESSAGE =
  "This file was selected but not uploaded. Please choose it again to upload.";
...
const NAVIGATION_WARNING_COPY =
  "Uploads may be interrupted if you leave before they finish. Documents already processing will continue in the background.";
...
export function shouldWarnBeforeUnloadForUploadItems(
  items: Array<Pick<UploadItem, "status" | "documentId" | "file">>
) {
  return items.some(
    (item) =>
      Boolean(item.file) &&
...
export function buildCreateQuestionsUrl(params: {
  documentIds: string[];
}) {
  const searchParams = new URLSearchParams();

  params.documentIds.forEach((documentId) => {
    searchParams.append("selectedDocumentId", documentId);
  });

  return `/dashboard/generate?${searchParams.toString()}`;
}

export const buildAutogenerateQuestionsUrl = buildCreateQuestionsUrl;
export const buildAutogeneratePracticeUrl = buildCreateQuestionsUrl;
...
export function UploadForm({
  ...
  useEffect(() => {
    ...
    if (hasInterruptedItems) {
      setUploadStatus("Some files were selected but not uploaded. Please choose them again to upload.");
      return;
    }
    if (hasCheckingItems) {
  ...
  const shouldWarnBeforeUnload = shouldWarnBeforeUnloadForUploadItems(items);
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !shouldWarnBeforeUnload) return;

    const warnBeforeUnload = (event: BeforeUnloadEvent) => {
      event.preventDefault();
      event.returnValue = NAVIGATION_WARNING_COPY;
    };
  ...
  const generateFromReadyDocuments = () => {
    ...
    router.push(buildCreateQuestionsUrl({ documentIds: readyDocumentIds }));
  };
    ...
    >
      ...
      {items.length > 0 ? (
        <div className="space-y-3 rounded-md border border-ink/10 bg-ink/[0.02] p-3">
          ...
          {summary.interrupted > 0 ? (
            <p className="text-xs text-ink/60">Not uploaded: {summary.interrupted}</p>
          ) : null}
          {loading ? (
            <p className="text-xs text-ink/60">
              ...
              {" "}{NAVIGATION_WARNING_COPY}
            </p>
          ) : null}
          {!loading && hasInterruptedItems ? (
            <p className="text-xs text-ink/60">
              Some files were selected but not uploaded. Please choose them again to upload.
            </p>
          ) : null}
          ...
          {hasCompletedBatch && summary.failed > 0 && summary.ready > 0 ? (
            <p className="text-xs text-ink/60">
              Some files failed. You can still create questions from the ready documents.
            </p>
          ) : null}
          {!loading && summary.processing > 0 && summary.ready > 0 ? (
            <p className="text-xs text-ink/60">
              Some files are still processing. You can create questions from the ready documents now, or wait.
            </p>
          ) : null}
        </div>
      ) : null}
      ...
      {canGenerateFromReadyDocuments && readyDocumentIds.length > 1 ? (
        <Button type="button" onClick={generateFromReadyDocuments} data-testid="generate-ready-documents">
          Create questions from these materials
        </Button>
      ) : null}
      {canGenerateFromReadyDocuments && readyDocumentIds.length === 1 ? (
        <Button type="button" onClick={generateFromReadyDocuments} data-testid="generate-ready-documents">
