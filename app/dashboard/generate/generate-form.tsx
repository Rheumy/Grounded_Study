...
function getSelectedDocumentIds(searchParams: URLSearchParams): string[] {
  const repeatedIds = searchParams.getAll("selectedDocumentId");
  const pluralIds = (searchParams.get("selectedDocumentIds") ?? "")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean);
...
export function GenerateForm({
  ...
  useEffect(() => {
    const queryDocumentIds = getSelectedDocumentIds(searchParams);
    ...
    if (documentIds.length > 0) {
      setSelectedDocs(documentIds);
      setStatus("Selected uploaded material. Choose your question settings, then generate questions.");
      setError(null);
    } else {
      setError("No ready documents selected.");
    }
    router.replace("/dashboard/generate");
  }, [documents, router, searchParams]);
